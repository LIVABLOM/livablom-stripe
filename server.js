...
app.post("/webhook", bodyParser.raw({ type: "application/json" }), async (req,res)=>{
  ...
  let event;
  try { event = stripe.webhooks.constructEvent(req.body, sig, stripeWebhookSecret); }
  ...
  if(event.type === "checkout.session.completed"){
    ...
    try{
      ...
      console.log("✅ Réservation ajoutée en BDD:", s.metadata);
    }catch(dbErr){ console.error("❌ Erreur BDD:", dbErr); }
  }
...
app.get("/api/reservations/:logement", async (req,res)=>{
  ...
  try{
    ...
    const events = result.rows.map(r=>({ start:r.date_debut, end:r.date_fin, display:"background", color:"#ff0000", title:"Réservé"}));
    res.json(events);
  }catch(err){
...
app.post("/api/checkout", async (req,res)=>{
  try{
    ...
    // Mode test : override montant à 1 € si TEST_PAYMENT=true
    if(isTest && process.env.TEST_PAYMENT === "true") amount = 1;

    const session = await stripe.checkout.sessions.create({
      payment_method_types:["card"],
      line_items:[{
        price_data:{currency:"eur", product_data:{name:`Réservation ${logement}`}, unit_amount:amount*100},
        quantity:1
      }],
      mode:"payment",
...
// Démarrage serveur
app.listen(port, ()=>console.log(`🚀 Serveur lancé sur port ${port} en mode ${isTest?"TEST":"PRODUCTION"}`));
