...
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // BDD
    try { await insertReservation(logement, email, startDate.toISOString(), endDate.toISOString()); }
    ...
    // Email via Brevo à deux destinataires (admin + pro)
...
app.get('/api/reservations/:logement', async (req, res) => {
  ...
  if (CALENDAR_URL) {
    try {
      const r = await axios.get(`${CALENDAR_URL}/api/reservations/${logement}`);
